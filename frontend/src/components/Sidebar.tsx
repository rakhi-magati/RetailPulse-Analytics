...
const navItems: NavItem[] = [
  ...
  { label: "Products", icon: <ProductsIcon />, path: "/dashboard" },
  { label: "Customers", icon: <CustomersIcon />, path: "/dashboard" },
  { label: "Inventory", icon: <InventoryIcon />, path: "/dashboard" },
  ...
  return (
    ...
    >
      ...
      <List sx={{ px: 1.5, py: 1.5, flexGrow: 1 }}>
        {navItems.map((item) => {
          const selected = location.pathname === item.path && item.label === "Dashboard";
          return (
            <ListItemButton
              key={item.label}
              selected={selected}
