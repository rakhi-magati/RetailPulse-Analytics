...
async def upload(request:Request,import_type:str,file:UploadFile=File(...),db:Session=Depends(get_db),company_id:int=Depends(get_current_company_id),user:User=Depends(get_current_user),_=AdminOnly):
    ...
    return data_import_service.upload(db,company_id,user.id,import_type,file.filename,source,get_client_ip(request),get_client_browser(request))

@router.post("/{job_id}/process",response_model=ImportOut)
def process(job_id:int,request:Request,db:Session=Depends(get_db),company_id:int=Depends(get_current_company_id),user:User=Depends(get_current_user),_=AdminOnly): return data_import_service.process(db,company_id,job_id,user.id,get_client_ip(request),get_client_browser(request))
